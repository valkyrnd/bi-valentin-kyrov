...
- Логическое поле подключается путем добавления логической связи к объекту таблицы. Физическое поле, в отличие от логического, относится к содержимому исходной таблицы, а логическое расширяет поля данными из других таблиц
...
- Поле o2o позволяет использовать связанные данные, полученные путем присоединения таблицы в объекте, расширяя её. Через foreign key можно привязать только статические объекты, а через o2o - ещё и вычисляемые динамически на основании условий. Также, связь через foreign key используется для обеспечения ссылочной целостности, необходимой, например, при каскадном удалении файлов.
### Что такое справочные данные (мастер-данные) в DWH
-  *Мастер-данные* — это данные, содержащие ключевую информацию о бизнесе, в том числе о клиентах, о продуктах, о работниках, о технологиях и материалах. Представляются в форме справочников или иерархических схем.
